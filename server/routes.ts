...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.get("/api/temp-fix-ghost/:phone", async (req, res) => {
    const secret = req.query.secret;
    if (secret !== "saman-fix-2026-ghost") return res.status(403).json({ error: "Forbidden" });
    ...
    try {
      ...
      await db.execute(sql`DELETE FROM login_events WHERE user_id = ${user.id}`);
      await db.execute(sql`DELETE FROM user_views WHERE user_id = ${user.id}`);
      await db.delete(users).where(eq(users.id, user.id));
      res.json({ message: `Ghost account ${phone} deleted`, userId: user.id });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  });
  ...
  app.post("/api/checkout", paymentLimiter, isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      if (telrResult.order?.url) {
        // Store both cartId and Telr order.ref (separated by ::) so we can find by cartId and check with order.ref
        await storage.updateTransactionReference(transaction.id, `${cartId}::${telrResult.order.ref}`);
  ...
  app.get("/api/payment/verify", isAuthenticated, async (req, res) => {
    ...
    try {
      // Find the pending transaction by cartId
      const transaction = await storage.getTransactionByReference(cart);
      if (!transaction) {
        console.log("[TELR] Transaction not found for cart:", cart);
      ...
      console.log("[TELR] Using order_ref:", telrOrderRef, "from paymentReference:", transaction.paymentReference);

      // Check order status with Telr using the actual Telr order reference
      const telrParams = new URLSearchParams({
        ivp_method: "check",
        ivp_store: telrStoreId,
        ivp_authkey: telrAuthKey,
  ...
  app.post("/api/admin/seed-demo", isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      // Packages are managed exclusively via admin panel - never seed them here
  ...
  // Legacy cleanup disabled - packages managed exclusively via admin panel
