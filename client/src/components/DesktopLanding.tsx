...
function SearchPanel(p: SearchPanelProps) {
  ...
  return <div className="absolute inset-x-4 -bottom-[87px] z-20 rounded-lg border border-[#d9dddf] bg-white p-2.5 shadow-[0_12px_30px_rgba(32,48,58,.18)] sm:inset-x-10">
    ...
    <div className="mt-2 grid gap-1.5 sm:grid-cols-[1.6fr_1fr_1fr_1fr_auto]">
      ...
      <Select value={p.brand} onValueChange={(v) => { p.setBrand(v); p.setModel("All"); }}><SelectTrigger className="h-8 border-[#dfe3e5] bg-white text-[11px] text-[#101820]" aria-label={labels.category}><SelectValue placeholder={labels.category} /></SelectTrigger><SelectContent><SelectItem value="All">{p.market === "automotive" ? labels.allMakes : labels.allCategories}</SelectItem>{p.brands.map((b) => <SelectItem key={b} value={b}>{b}</SelectItem>)}</SelectContent></Select>
      {p.market === "automotive" && p.models.length > 0 && <ModelCombobox models={p.models} value={p.model} onValueChange={p.setModel} emptyValue="All" emptyLabel={labels.allModels} searchPlaceholder={labels.modelSearch} ariaLabel={labels.modelAria} className="h-8 text-[11px]" />}
      <Input value={p.priceMax} onChange={(e) => p.setPriceMax(e.target.value)} type="number" placeholder={labels.price} aria-label={labels.priceAria} className="h-8 border-[#dfe3e5] bg-white text-[11px] text-[#101820]" />
      <Link href={p.searchUrl} data-testid="desktop-search-submit" className="flex h-8 items-center justify-center gap-1 rounded-md bg-[#f45b27] px-4 text-[11px] font-bold text-white hover:bg-[#df4818]"><Search className="h-3.5 w-3.5" />{p.ar ? "بحث" : "Search"}</Link>
    </div>
